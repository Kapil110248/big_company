...
export const MyManagementPage: React.FC = () => {
  ...
  const supplierColumns = [
    {
      ...
      render: (_: any, record: Supplier) => (
        <Space>
          <Avatar style={{ backgroundColor: record.type === 'manufacturer' ? '#722ed1' : '#1890ff' }}>
            {record.type === 'manufacturer' ? 'M' : 'S'}
          </Avatar>
          <div>
            <Text strong>{record.name}</Text>
            <br />
            <Tag color={record.type === 'manufacturer' ? 'purple' : 'blue'}>{record.type.toUpperCase()}</Tag>
          </div>
        </Space>
      ),
    },
    ...
    },
    {
      ...
      render: (v: number) => (
        <Text style={{ color: v > 0 ? '#ff4d4f' : '#52c41a' }}>{formatCurrency(v)}</Text>
      ),
    },
    {
      title: 'Status',
      ...
      render: (_: any, record: Supplier) => (
        <Space>
          <Button
            size="small"
            icon={<EyeOutlined />}
            onClick={() => { setSelectedSupplier(record); setSupplierModalOpen(true); }}
          >
            View
          </Button>
  ...
  const invoiceColumns = [
    ...
    },
    {
      ...
      render: (v: number) => <Text style={{ color: '#ff4d4f' }}>-{formatCurrency(v)}</Text>,
    },
    {
      title: 'Net Profit',
      dataIndex: 'net_profit',
      key: 'net_profit',
      render: (v: number) => <Text strong style={{ color: '#722ed1', fontSize: 16 }}>{formatCurrency(v)}</Text>,
    },
    {
      title: 'Status',
      dataIndex: 'status',
      key: 'status',
      render: (status: string) => {
        const colors: Record<string, string> = { paid: 'green', pending: 'orange', disputed: 'red' };
    ...
    },
    {
      ...
      render: (_: any, record: ProfitInvoice) => (
        <Button
          size="small"
          icon={<EyeOutlined />}
          onClick={() => { setSelectedInvoice(record); setInvoiceModalOpen(true); }}
        >
          View Details
        </Button>
      ),
    ...
    <div>
      ...
      <Card>
        <Tabs defaultActiveKey="suppliers">
          <TabPane
            tab={<span><TruckOutlined /> Suppliers & Manufacturers ({suppliers.length})</span>}
            key="suppliers"
          >
            <Alert
            ...
            <Space style={{ marginBottom: 16 }}>
              <Input
                ...
                style={{ width: 250 }}
              />
              <Button type="primary" icon={<PlusOutlined />} onClick={() => setAddSupplierModalOpen(true)}>
                Add Supplier
              </Button>
            </Space>
            <Table
          ...
          <TabPane
            tab={<span><FileTextOutlined /> Profit Invoices ({invoices.length})</span>}
            key="invoices"
          >
            <Alert
      ...
      <Modal
        ...
        width={700}
        footer={[
          <Button key="close" onClick={() => setSupplierModalOpen(false)}>Close</Button>,
          <Button key="edit" type="primary" icon={<EditOutlined />}>Edit Supplier</Button>,
        ]}
      >
        {selectedSupplier && (
          <>
            <Descriptions bordered column={2}>
              <Descriptions.Item label="Company Name" span={2}>
                <Space>
                  <Avatar style={{ backgroundColor: selectedSupplier.type === 'manufacturer' ? '#722ed1' : '#1890ff' }} size="large">
                    {selectedSupplier.type === 'manufacturer' ? 'M' : 'S'}
                  </Avatar>
                  <div>
                    <Text strong style={{ fontSize: 16 }}>{selectedSupplier.name}</Text>
                    <br />
                    <Tag color={selectedSupplier.type === 'manufacturer' ? 'purple' : 'blue'}>
                      {selectedSupplier.type.toUpperCase()}
                    </Tag>
                  </div>
                </Space>
              </Descriptions.Item>
              <Descriptions.Item label={<><PhoneOutlined /> Contact Person</>}>
              ...
              <Descriptions.Item label="Total Orders">{selectedSupplier.total_orders}</Descriptions.Item>
              <Descriptions.Item label="Products Supplied">{selectedSupplier.products_supplied}</Descriptions.Item>
              <Descriptions.Item label="Total Paid">
                <Text style={{ color: '#52c41a' }}>{formatCurrency(selectedSupplier.total_paid)}</Text>
              </Descriptions.Item>
              <Descriptions.Item label="Outstanding Balance">
                <Text style={{ color: selectedSupplier.outstanding_balance > 0 ? '#ff4d4f' : '#52c41a' }}>
                  {formatCurrency(selectedSupplier.outstanding_balance)}
                </Text>
              </Descriptions.Item>
              <Descriptions.Item label="Status" span={2}>
                <Tag color={selectedSupplier.status === 'active' ? 'green' : 'default'}>
      ...
      <Modal
        ...
        width={600}
        footer={[
          <Button key="close" onClick={() => setInvoiceModalOpen(false)}>Close</Button>,
          selectedInvoice?.status === 'pending' && (
            <Button key="pay" type="primary" icon={<DollarOutlined />}>Mark as Paid</Button>
          ),
        ].filter(Boolean)}
      >
        {selectedInvoice && (
          <>
            <Descriptions bordered column={1}>
              <Descriptions.Item label="Invoice Number">
                <Tag color="purple" style={{ fontSize: 14 }}>{selectedInvoice.invoice_number}</Tag>
              </Descriptions.Item>
              <Descriptions.Item label="Period">
                <Text strong>{selectedInvoice.period}</Text>
              </Descriptions.Item>
              <Descriptions.Item label="Gross Profit">
                <Text style={{ color: '#52c41a', fontSize: 16 }}>{formatCurrency(selectedInvoice.gross_profit)}</Text>
              </Descriptions.Item>
              <Descriptions.Item label="Monthly Expenses">
                <Text style={{ color: '#ff4d4f', fontSize: 16 }}>-{formatCurrency(selectedInvoice.monthly_expenses)}</Text>
              </Descriptions.Item>
              <Descriptions.Item label="Net Profit">
                <Text strong style={{ color: '#722ed1', fontSize: 20 }}>{formatCurrency(selectedInvoice.net_profit)}</Text>
              </Descriptions.Item>
              ...
              <Descriptions.Item label="Due Date">{formatDate(selectedInvoice.due_date)}</Descriptions.Item>
              {selectedInvoice.paid_at && (
                <Descriptions.Item label="Paid Date">{formatDate(selectedInvoice.paid_at)}</Descriptions.Item>
              )}
              {selectedInvoice.admin_notes && (
                <Descriptions.Item label="Admin Notes">{selectedInvoice.admin_notes}</Descriptions.Item>
              )}
            </Descriptions>
          </>
        )}
      ...
      <Modal
        ...
        onCancel={() => { setAddSupplierModalOpen(false); form.resetFields(); }}
        footer={null}
        width={700}
      >
        <Form form={form} layout="vertical" onFinish={handleAddSupplier}>
          <Row gutter={16}>
            ...
            <Col span={12}>
              <Form.Item name="payment_terms" label="Payment Terms" rules={[{ required: true }]}>
                <Select size="large">
          ...
          <Form.Item>
            <Space style={{ width: '100%', justifyContent: 'flex-end' }}>
              <Button onClick={() => { setAddSupplierModalOpen(false); form.resetFields(); }}>Cancel</Button>
